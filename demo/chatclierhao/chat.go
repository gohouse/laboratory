...
var (
	rds *redis.Client
	u   *User
	ch  = fmt.Sprintf("p_%s_ch", config.AppName)       // redis 广播频道名字
	chatKey = fmt.Sprintf("z_%s_chat", config.AppName) // z:chat:gasii
	idKey      = fmt.Sprintf("a_%s_id", config.AppName)
	keyVersion = fmt.Sprintf("h_%s_version", config.AppName) // hset version no xxx; hset version url xxx
)
